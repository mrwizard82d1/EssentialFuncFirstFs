...
module TestHelpers =
    ...
    let isCustomerAlreadyExistsException message customerId =
        let expected = $"Customer '{customerId}' already exists."
        match message with
        | s when s = expected ->
            passTest
        | _ -> failTest $"%A{message} not expected."
...
module ``Convert customer to eligible`` =
    ...
    let ``should succeed if customer does not exist`` () =
        ...
        | Ok customer ->
            customer
            |> should equal { CustomerId = name; IsRegistered = true; IsEligible = false }
    ...
    let ``should fail if customer does exist`` () =
        ...
        match result with
        | Error ex ->
            isCustomerAlreadyExistsException ex.Message name
        | Ok customer ->
            failTest $"{customer} was not expected."
